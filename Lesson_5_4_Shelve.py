...
"""Для збереження введених даних на диску за допомогою модулю shelve було написано дві функції: save_data() та 
get_data() (рядки програми 15-23). А також додано їх виклики у відповідніх місцях програми (рядки 152 та 228)"""
...
def save_data(accounts_list_, posts_):
    with shelve.open(filename) as a_l:
        a_l['accounts_list'] = accounts_list_
        a_l['posts'] = posts_


def get_data():
    with shelve.open(filename) as a_l:
        return a_l['accounts_list'], a_l['posts']
...
try:
    accounts_list, posts = get_data()
except KeyError:
    print("При спробі програми завантажити попередньо збережені на диску дані контактів з групи файлів різних розширень"
          "\n' ListKeeper.***' файли не було знайдено. При першому збереженні введених даних вказані файли з'являться"
          "\nу папці, в якій запущений срипт. В подальшому стартові дані контактів та постів, збережених при попередніх"
          "\nвиконаннях програми, будуть завантажуватись при запуску програми вже з файлів 'ListKeeper.***'")
